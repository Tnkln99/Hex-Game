...
class BackTrack(AI):
	...
	def backtrack(self, graph, depth, isMaximizing):
		...
		if isMaximizing:
			...
			for i in range(self.size*self.size):
				if i not in graphCopyc.getGraphComplet():
					...
					score = self.backtrack(graphCopyc, depth+1, False)
			...
			for i in range(self.size*self.size):
				if i not in graphCopyc.getGraphComplet():
					...
					score = self.backtrack(graphCopyc, depth+1, True)
	...
	def algo(self, GameGraph):
		...
		for i in range(self.size*self.size):
			if i not in graphCopy.getGraphComplet().keys():
				...
				score = self.backtrack(graphCopy, 0, False)
				if score > bestScore:
					bestScore = score
					move = i
